...
module.exports = {
  ...
  //Checking is tx valid datatx transaction
  get_datatx: function(txid, cb) {
    // var cmd = prepareRpcCommand(settings.api_cmds.getdatatx, (txid ? [txid] : []));
    var split = settings.api_cmds.getdatatx.split(' ');
    var cmd = {method: split[0], parameters: [split[1],txid]};

    if (!(settings.api_cmds.getdatatx == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
          // check if an error msg was received from the rpc server
          if (response == 'There was an error. Check your console.')
        ...
        var uri = base_url + 'getdatatx?txid=' + (txid ? txid : '');
  ...
  get_supply: function(cb) {
    ...
    } else if (settings.sync.supply == 'GETINFO1') {
