...
module.exports = {
  ...
  get_last_txs: function(start, length, min, internal, ntype, cb) {
    this.get_last_txs_ajax(start, length, min, ntype, function(txs, count) {
      var data = [];
  ...
  get_last_txs_ajax: function(start, length, min, ntype = -1, cb) {
    switch (+ntype) {
      case 7:
        Tx.find({'total': {$gte: min}, 'ntype': ntype}).countDocuments().then((count) => {
          // get last transactions where there is at least 1 vout
          Tx.find({'total': {$gte: min}, 'vout': { $gte: { $size: 1 }}, 'ntype': ntype}).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).exec().then((txs) => {
            return cb(txs, count);
          }).catch((err) => {
            return cb(err);
          });
